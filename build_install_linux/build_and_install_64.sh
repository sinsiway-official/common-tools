...
assign_directories() {
    ...
    if [ -z "$dest_dir" ]; then
        echo "destination directories must be specified ... FAILED"
        display_help
        return 1
    fi

    if [ -d "$dest_dir" ]; then
        echo "Destination directory '$dest_dir' exists. please remove it before running the script."
    else
    ...
    target_tools_dir="$dest_dir"

    echo "Directories assigned successfully."
    echo "Install directory: $target_tools_dir"
}
...
install_package_openssl() {
    ...
    cp -R $petra_tools_install_dir/$package_name/include/* $petra_tools_include_dir
}
...
extracted_installed_objects() {
    ...
    for installed_package in $installed_package_list; do
        local installed_static_library_list=$(ls $petra_tools_install_dir/$installed_package/lib/*.a)
        for installed_static_library in $installed_static_library_list; do
...
make_libraries() {
    ...
    mkdir -p $target_tools_dir/include
    mkdir -p $target_tools_dir/lib

    cp -fR $petra_tools_include_dir/* $target_tools_dir/include
